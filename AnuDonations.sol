...
abstract contract AccountBoundNFT is Context, ERC165, IAccountBoundNFT, IERC721Metadata, IERC721Errors {
    ...
    function _mint(address to, uint256 tokenId) internal {
        if (to == address(0) || _owners[tokenId] != address(0)) {
...
contract AnuDonations is AccountBoundNFT, Ownable, IDonationReceipt {
    ...
    mapping(uint256 => Donation) public donation;
    mapping(address => uint256[]) public accountDonations;
    ...
    modifier onlyRegistry() {
        require(_msgSender() == registry, "ANU - Registry only.");
        _;
    }

    constructor() Ownable(msg.sender) AccountBoundNFT('ANU Donation Receipt', 'ANU DONATION') {}

    /**
     * @notice Set Registry contract with `registryContract` address.
     * 
    ...
    function setRegistry(
        ...
        require(registryContract != address(0), "ANU - Invalid Registry.");

        registry = registryContract;
    }

    /**
     * @notice Mint Donation Receipt NFT for `account` for donation `amount`.
     * 
     * - Only callable by Registry contract.
    ...
    function mintReceipt(
        ...
        require(account != address(0), "ANU - Invalid account.");
    	require(amount > 0, "ANU - Invalid amount.");

        donationIndex += 1;
        donation[donationIndex] = Donation(
            account,
            amount
        );
        accountDonations[account].push(donationIndex);
    	_mint(account, donationIndex);

        return donationIndex;
    }

    /**
     * @notice Returns Donation Amount for Donation Receipt `tokenId`
     *
     */
    function donationAmount(
        uint256 tokenId
    ) public view returns (uint256 amount) {
        amount = donation[tokenId].amount;
    }

    /**
     * @notice Returns Donation Account for Donation Receipt `tokenId`
     *
     */
    function donationAccount(
        uint256 tokenId
    ) public view returns (address account) {
        account = donation[tokenId].account;
    }

    /**
     * @notice Returns number of donations by `account`.
     *
     */
    function donationCount(
        address account
    ) external view returns (uint256 amount) {
        amount = accountDonations[account].length;
    }

    /**
     * @notice Returns all donation ids by `account`.
     *
     */
    function donationsByAccount(
        address account
    ...
    function tokenURI(
        ...
        string memory _donation = toString(donationAmount(tokenId) / 10 ** 18);
        string memory svg = string(abi.encodePacked(
            ...
            '<text x="244" y="84" class="base1">Donation Receipt #',
            toString(tokenId),
            '</text><text x="244" y="300" class="base0">Donation: ',
            _donation,
            ' USDC</text></svg>'
        ));
        string memory json = base64(bytes(string(abi.encodePacked(
            '{"name": "Anu Initiative Donation Receipt #',
            toString(tokenId),
            '", "description": "Anu Initiative Donation Receipt", "image": "data:image/svg+xml;base64,',
            base64(bytes(svg)), '"}'
        ))));
